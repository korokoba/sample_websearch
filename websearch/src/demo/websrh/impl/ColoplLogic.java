 ...
 *
 */
public class ColoplLogic extends ClientLogic{
	 ...
	 * @param argData
	 */
	public ColoplLogic(ToolArgData argData) {
		super(argData);
		this.client = new WebClient(argData.getTimeout());
	}

	private static final Logger log = Logger.getLogger( ColoplLogic.class );
		...
		(new FileWriter(new File(getFileName())){
			...
			protected String edit(String text) {
				String date = (new Html(text))
						.doFilter("<span\\s*class=\"date\">","</span>",true)
						.getElement();
				String title = (new Html(text))
						.doFilter("<span\\s*class=\"txt\">","</span>",true)
						.getElement();
				log.info(date+","+title);
				return date+","+title;
			}
	...
	private List<String> getPageInfo(final int pageMo) throws Exception {
		...
		List<String> list = (new Html(page))
				.doFilter("<div\\s*class=\"unitNewsItem\">", "</div>")
				.doFilter("<li>", "</li>").
				getElementsList();
		for(String e : list){
